...
local function info(w, h)
    ...
    local json, err = mp.utils.format_json({ width = w, height = h, disabled = disabled, available = true,
        socket = options.socket, thumbnail = options.thumbnail, overlay_id = options.overlay_id })
    if pre_0_30_0 then
...
local function draw(w, h, script)
    ...
    elseif script then
        local json, err = mp.utils.format_json({ width = w, height = h, x = x, y = y, socket = options.socket,
            thumbnail = options.thumbnail, overlay_id = options.overlay_id })
